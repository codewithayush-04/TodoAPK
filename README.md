![React](https://img.shields.io/badge/React-Frontend-blue?logo=react)
![Supabase](https://img.shields.io/badge/Supabase-Backend-green?logo=supabase)
![Tailwind](https://img.shields.io/badge/TailwindCSS-Styling-38B2AC?logo=tailwindcss)
![Status](https://img.shields.io/badge/Status-Active-success)

A modern **dark-themed Kanban task manager** built using **React + Supabase**, inspired by Trello. Designed for productivity, smooth UI/UX, and a strong developer portfolio.

---

## 🌐 Live Demo

👉 https://todo-apk-seven.vercel.app/

---

## 📸 Screenshots
<img width="1172" height="725" alt="task" src="https://github.com/user-attachments/assets/51360a39-47ae-4f9c-be9e-04eda8dd8629" />

### 🏠 Dashborad

<img width="1251" height="569" alt="Screenshot 2026-04-07 at 9 08 12 PM" src="https://github.com/user-attachments/assets/0d14ef75-801c-410a-8543-5a3d7a4f937d" />


### 📋 Board View

<img width="1218" height="574" alt="Screenshot 2026-04-07 at 9 07 54 PM" src="https://github.com/user-attachments/assets/c86aa716-3d1f-487c-b208-725eaaebf8cf" />

### ➕ Add Task Modal


---

## ✨ Features
Future Requirements
- 🔐 **Authentication**
  - Secure login & signup using Supabase Auth

- 📋 **Boards Management**
  - Create and manage multiple boards

- 🧩 **\Task Grid Workflow**
  - Columns: **To Do / In Progress / Done**

- ✅ **Task Management**
  - Add, edit, and delete tasks  
  - Title, description, due date, priority

- 🔄 **Drag & Drop**
  - Smoothly move tasks across columns

- 🔍 **Search & Filters**
  - Search tasks instantly  
  - Filter by priority and due date

- ⚡ **Realtime Updates (Optional)**
  - Sync changes using Supabase Realtime

- 🔔 **User Experience**
  - Toast notifications  
  - Loading states  
  - Smooth animations  
  - Fully responsive design

---

## 🛠️ Tech Stack

- **Frontend**: React (Vite)
- **Styling**: Tailwind CSS (Dark Theme)
- **Backend**: Supabase (Auth + Database + Realtime)
- **Drag & Drop**: react-beautiful-dnd

---
```
## 📂 Project Structure
src/
├── components/
├── pages/
├── hooks/
├── supabase/
├── utils/

---

## ⚙️ Setup (Supabase)

1. Create a project on **Supabase**
2. Open **SQL Editor**
3. Run:npm run  dev

4. Enable authentication:
- Go to **Authentication → Providers**
- Enable **Email**

---

## 💻 Local Development

### 1. Clone the repository
```bash
git clone https://github.com/codewithayush-04/task-grid.git
cd task-grid
cp .env.example .env

VITE_SUPABASE_URL=your_supabase_url
VITE_SUPABASE_ANON_KEY=your_anon_key
npm install
npm run dev
npm run build
npm run preview

🎨 UI Highlights
	•	🌑 Modern dark/black theme
	•	🧊 Glassmorphism design
	•	⚡ Smooth transitions & animations
	•	📱 Fully responsive layout
   
🚀 Future Enhancements
	•	👥 Team collaboration
	•	📎 File attachments
	•	🧠 AI-based task suggestions
	•	📊 Analytics dashboard
```
   👨‍💻 Author

Ayush Gupta

